...
2. **設計書の該当節だけを読む。** まずタスクのフェーズを決め、対応する設計書（Phase 1 は `design-local-phase1.md`、Phase 2 以降は `design-local-phase2-client.md`・`design-local-phase2-server.md` など `design-local-phase*.md` の該当ファイル）を選ぶ。`grep -n "^# \|^## " <設計書>` で節の位置を特定し、その節と、参照されている節（Phase 1 なら型定義は §7、テストハーネスは §24.1）だけを読む。全体を通読しない。
